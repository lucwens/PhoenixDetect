...
static bool HasError(const std::vector<HHD_ValidationIssue> &issues, const std::string &substring)
{
    return std::any_of(issues.begin(), issues.end(),
                       [&](const HHD_ValidationIssue &i) { return i.severity == HHD_IssueSeverity::Error && i.message.find(substring) != std::string::npos; });
}
...
static bool HasWarning(const std::vector<HHD_ValidationIssue> &issues, const std::string &substring)
{
    return std::any_of(issues.begin(), issues.end(), [&](const HHD_ValidationIssue &i)
                       { return i.severity == HHD_IssueSeverity::Warning && i.message.find(substring) != std::string::npos; });
}

static int CountErrors(const std::vector<HHD_ValidationIssue> &issues)
{
    return static_cast<int>(std::count_if(issues.begin(), issues.end(), [](const HHD_ValidationIssue &i) { return i.severity == HHD_IssueSeverity::Error; }));
}

static int CountWarnings(const std::vector<HHD_ValidationIssue> &issues)
{
    return static_cast<int>(std::count_if(issues.begin(), issues.end(), [](const HHD_ValidationIssue &i) { return i.severity == HHD_IssueSeverity::Warning; }));
}
...
TEST_CLASS(ValidationErrors){public : TEST_METHOD(EmptyMarkers){auto issues = ValidateMeasurementSetup(10, {});
...
TEST_CLASS(ValidationWarnings){public : TEST_METHOD(LedOverheating){auto issues = ValidateMeasurementSetup(120, {ValidMarker()});
...
TEST_CLASS(ValidationValid){public : TEST_METHOD(ValidSetup){std::vector<HHD_MarkerEntry> markers = {{1, 1, 1}, {1, 2, 1}, {2, 1, 1}};
auto issues = ValidateMeasurementSetup(10, markers);
